...
open class PagingCollectionViewLayout<T: PagingItem>: UICollectionViewFlowLayout where T: Equatable {
  ...
  fileprivate var range: CountableRange<Int> {
  ...
  open override func layoutAttributesForDecorationView(ofKind elementKind: String, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
    ...
    if elementKind == PagingIndicatorView.reuseIdentifier {
      ...
      let from = PagingIndicatorMetric(
        frame: indicatorFrameForIndex((currentIndexPath as NSIndexPath).item),
        insets: indicatorInsetsForIndex((currentIndexPath as NSIndexPath).item))
      
      let to = PagingIndicatorMetric(
        frame: indicatorFrameForIndex((upcomingIndexPath as NSIndexPath).item),
        insets: indicatorInsetsForIndex((upcomingIndexPath as NSIndexPath).item))
  ...
  fileprivate func upcomingIndexPathForIndexPath(_ indexPath: IndexPath) -> IndexPath {
    ...
    } else if (indexPath as NSIndexPath).item == range.startIndex {
      return IndexPath(item: (indexPath as NSIndexPath).item - 1, section: 0)
    } else if (indexPath as NSIndexPath).item == range.endIndex {
      return IndexPath(item: (indexPath as NSIndexPath).item + 1, section: 0)
    }
    return indexPath
  }
  ...
  fileprivate func indicatorFrameForIndex(_ index: Int) -> CGRect {
    ...
    if index < range.startIndex {
      let frame = frameForIndex((currentIndexPath as NSIndexPath).item)
